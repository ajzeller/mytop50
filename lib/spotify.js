...
const SpotifyProvider = (props) => {
  ...
  const getTracks = async (refresh_token, time_range, limit) => {
    try{
      ...
      const res = await fetch(
        `http://localhost:3000/api/tracks?refresh_token=${refresh_token}&time_range=${time_range}&limit=${limit}`
      )
  ...
  const getArtists = async (refresh_token, time_range, limit) => {
    try{
      ...
      const res = await fetch(
        `http://localhost:3000/api/artists?refresh_token=${refresh_token}&time_range=${time_range}&limit=${limit}`
      )
      const json = await res.json()
      console.log(json)
      return(json)
  ...
  const getHistory = async (refresh_token, limit) => {
    try{
      ...
      const res = await fetch(
        `http://localhost:3000/api/history?refresh_token=${refresh_token}&limit=${limit}`
      )
      const json = await res.json()
      console.log(json)
      return(json)
  ...
  const getProfile = async (refresh_token) => {
    try{
      ...
      const res = await fetch(
        `http://localhost:3000/api/profile?refresh_token=${refresh_token}`
      )
      const json = await res.json()
      // console.log(json)
      return(json)
